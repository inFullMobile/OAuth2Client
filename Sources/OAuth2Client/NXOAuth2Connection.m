...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)theResponse; {
    ...
    if (self.statusCode == 401
        ...
        // Have got a www-authenticate header that isn't a 401. Assume it's a reportable problem
        if (responseHandler) {
            NSString *localizedError = [NSString stringWithFormat:NSLocalizedString(@"Auth failure: %@, HTTP Error: %d", @"NXOAuth2HTTPErrorDomain authentication error description"), authenticateHeader, self.statusCode];
            NSError *error = [self authenticationError:localizedError];
            responseHandler(theResponse, data, error);
        }
        [self cancel];
...
- (NSError *)authenticationError:(NSString *)localizedError {
    NSDictionary *errorUserInfo = @{NSLocalizedDescriptionKey: localizedError};
    return [NSError errorWithDomain:NXOAuth2HTTPErrorDomain code:self.statusCode userInfo:errorUserInfo];
}
